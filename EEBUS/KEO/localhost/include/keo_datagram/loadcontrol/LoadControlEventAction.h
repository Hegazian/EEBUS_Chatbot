...
#ifndef KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTACTION_H_
#define KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTACTION_H_
...
/**
 * Declaration LoadControlEventAction
 */
class LoadControlEventAction : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 6;

  //! Enumeration LoadControlEventAction::Value
  enum class Value {
    UNDEFINED__ = -1,
    PAUSE,
    RESUME,
   ...
   */
  LoadControlEventAction(LoadControlEventAction::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  LoadControlEventAction(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static LoadControlEventAction fromString(const std::string &c_text);
   ...
   */
  LoadControlEventAction::Value getValue() const;
   ...
   * @return enumeration value
   */
  LoadControlEventAction::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const LoadControlEventAction &other) const;
...
private:
  ...
  LoadControlEventAction::Value m_data;
...
/**
 * \deprecated Use LoadControlEventAction instead.
 */
using LoadControlEventActionStruct = LoadControlEventAction;
...
#endif // KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTACTION_H_