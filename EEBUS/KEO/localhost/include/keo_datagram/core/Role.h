...
#ifndef KEO_DATAGRAM_CORE_ROLE_H_
#define KEO_DATAGRAM_CORE_ROLE_H_
 ...
 * Declaration Role
 */
class Role : public SpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 3;

  //! Enumeration Role::Value
  enum class Value { UNDEFINED__ = -1, CLIENT, SERVER, SPECIAL };
  ...
  */
  Role(Role::Value c_data);

  /**
  * Construct from string.
  * @param c_text Classifier string
  */
  explicit Role(const std::string &c_text);
   ...
   */
  static Role::Value fromString(const std::string &c_text);
   ...
   */
  bool operator==(const Role &c_enumCmdClassifier) const;
...
#endif // KEO_DATAGRAM_CORE_ROLE_H_