...
#ifndef KEO_DATAGRAM_INCENTIVETABLE_INCENTIVEVALUETYPE_H_
#define KEO_DATAGRAM_INCENTIVETABLE_INCENTIVEVALUETYPE_H_
...
/**
 * Declaration IncentiveValueType
 */
class IncentiveValueType : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 4;

  //! Enumeration IncentiveValueType::Value
  enum class Value {
    UNDEFINED__ = -1,
    VALUE,
    AVERAGE_VALUE,
   ...
   */
  IncentiveValueType(IncentiveValueType::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  IncentiveValueType(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static IncentiveValueType fromString(const std::string &c_text);
   ...
   */
  IncentiveValueType::Value getValue() const;
   ...
   * @return enumeration value
   */
  IncentiveValueType::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const IncentiveValueType &other) const;
...
private:
  ...
  IncentiveValueType::Value m_data;
...
/**
 * \deprecated Use IncentiveValueType instead.
 */
using IncentiveValueTypeStruct = IncentiveValueType;
...
#endif // KEO_DATAGRAM_INCENTIVETABLE_INCENTIVEVALUETYPE_H_