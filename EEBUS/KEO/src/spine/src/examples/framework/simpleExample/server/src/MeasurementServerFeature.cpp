...
bool MeasurementServerFeature::handleMeasurementListDataRead(const keo_spine::DatagramRecord& datagramRecord)
{
    ...
    keo_datagram::Datagram pDatagram = datagramRecord.getDatagram().createReply(createMeasurementListData());
...
void MeasurementServerFeature::updateMeasurements()
{
    ...
    for (const auto& subscription : getSubscriptions())
    {
        if (isConnected(subscription.clientAddress.getDevice()))
        {
            ...
            sendDatagram(keo_datagram::Datagram(header, keo_datagram::Cmd(createMeasurementListData())));
        }
    }
}
...
keo_datagram::CmdDataPtr MeasurementServerFeature::createMeasurementListData() const
{
    DEBUG_FUNC();
    ...
    return std::make_shared<keo_datagram::MeasurementListData>(measurementDataList);
}
