...
MeasurementClientFeature::MeasurementClientFeature(const std::vector<keo_datagram::xs_unsignedInt>& entity)
    : ClientFeature()
{
    DEBUG_FUNC();
...
{
    DEBUG_FUNC();
    if (datagramRecord.getDatagram().isResult())
    {
        return handleResult(datagramRecord, memo);
    }
    if (datagramRecord.getDatagram().isReply())
    {
        switch (datagramRecord.getDatagram().getCmdDataType())
        {
            case keo_datagram::CmdData::Type::MEASUREMENT_LIST_DATA_TYPE:
            {
                LOG_INFO() << "MeasurementListReply " << datagramRecord.getDatagram().getSourceAddress().toString();
                handleMeasurementListData(std::dynamic_pointer_cast<keo_datagram::MeasurementListDataConst>(
                    datagramRecord.getDatagram().getCmdData()));
                return true;
            }
    ...
    const keo_spine::DatagramRecord& datagramRecord, const keo_spine::DatagramMemo& memo)
{
    ...
    {
        keo_datagram::ResultDataConstPtr subscriptionReply
        ...
        {
            LOG_INFO() << "Subscription rejected";
        }
        return true;
    }
    return false;
...
void MeasurementClientFeature::removeSubscriptions(const std::string& serverDeviceName)
{
    DEBUG_FUNC();
    for (std::map<keo_datagram::FeatureAddress, keo_datagram::NodeManagementSubscriptionRequestCallPtr>::iterator iter
         = subscriptionMap.begin();
         iter != subscriptionMap.end();)
    {
        std::map<keo_datagram::FeatureAddress, keo_datagram::NodeManagementSubscriptionRequestCallPtr>::iterator delIter
            = iter++;
        if (delIter->first.getDevice() == serverDeviceName)
        {
            subscriptionMap.erase(delIter);
        }
    }
}
    ...
    const keo_datagram::FeatureAddress& destination)
{
    ...
    keo_datagram::Datagram pDatagram = keo_datagram::Datagram(header, cmd);
    if (!pDatagram.isHeaderValid() || !pDatagram.isPayloadValid())
    {
        success = false;
        LOG_WARN() << "Created MeasurementListDataRead datagram is not valid";
    }

    if (!success)
    {
        LOG_WARN() << "Error while creating MeasurementListDataRead datagram";
    }

    return success ? pDatagram : keo_datagram::Datagram();
}
    ...
    const std::string& deviceName, const keo_datagram::DetailedDiscovery& detailedDiscovery)
{
    ...
    LOG_INFO() << "+++++++++++++ MeasurementClientFeature::onDetailedDiscovery() +++++++++++++";
