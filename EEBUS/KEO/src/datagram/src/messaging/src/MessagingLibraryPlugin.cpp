 ...
 ******************************************************************************
 */
class MessagingDatagramPlugin : public KeoDatagramPlugin {
public:
  MessagingDatagramPlugin();
  virtual ~MessagingDatagramPlugin();
...
KEO_DATAGRAM_PLUGIN(MessagingDatagramPlugin, "Messaging Datagram Plugin",
                    SPINE_VERSION_STRING, SPINE_VERSION_POSTFIX)

MessagingDatagramPlugin::MessagingDatagramPlugin() {
...
extern "C" KeoDatagramPlugin *createPlugin() {
  return new MessagingDatagramPlugin;
}

extern "C" void destroyPlugin(KeoDatagramPlugin *p) { delete p; }

CmdDataPtr
MessagingDatagramPlugin::createCmdData(const CmdData::Type type) const {
  switch (type) {
...
DataSelectorsPtr MessagingDatagramPlugin::createDataSelectors(
    const DataSelectors::Type type) const {
  switch (type) {
  case DataSelectors::Type::MESSAGING_LIST_DATA_SELECTORS_TYPE:
    return std::make_shared<MessagingListDataSelectors>();
...
DataElementsPtr MessagingDatagramPlugin::createDataElements(
    const DataElements::Type type) const {
  switch (type) {
  case DataElements::Type::MESSAGING_DATA_ELEMENTS_TYPE:
    return std::make_shared<MessagingDataElements>();
