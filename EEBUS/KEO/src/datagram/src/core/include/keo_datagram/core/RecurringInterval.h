...
#ifndef KEO_DATAGRAM_CORE_RECURRINGINTERVAL_H_
#define KEO_DATAGRAM_CORE_RECURRINGINTERVAL_H_
...
/**
 * Declaration RecurringInterval
 */
class RecurringInterval : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 7;

  //! Enumeration RecurringInterval::Value
  enum class Value {
    UNDEFINED__ = -1,
    YEARLY,
    MONTHLY,
   ...
   */
  RecurringInterval::Value getValue() const;
   ...
   * @return enumeration value
   */
  RecurringInterval::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const RecurringInterval &other) const;
...
private:
  ...
  RecurringInterval::Value m_data;
...
/**
 * \deprecated Use RecurringInterval instead.
 */
using RecurringIntervalStruct = RecurringInterval;
...
#endif // KEO_DATAGRAM_CORE_RECURRINGINTERVAL_H_