...
#ifndef KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESTATE_H_
#define KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESTATE_H_
...
/**
 * Declaration MeasurementValueState
 */
class MeasurementValueState : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 3;

  //! Enumeration MeasurementValueState::Value
  enum class Value { UNDEFINED__ = -1, NORMAL, OUT_OF_RANGE, ERROR };
   ...
   */
  MeasurementValueState(MeasurementValueState::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  MeasurementValueState(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static MeasurementValueState fromString(const std::string &c_text);
   ...
   */
  MeasurementValueState::Value getValue() const;
   ...
   * @return enumeration value
   */
  MeasurementValueState::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const MeasurementValueState &other) const;
...
private:
  ...
  MeasurementValueState::Value m_data;
...
/**
 * \deprecated Use MeasurementValueState instead.
 */
using MeasurementValueStateStruct = MeasurementValueState;
...
#endif // KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESTATE_H_