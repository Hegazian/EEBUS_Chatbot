...
#ifndef KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUETYPE_H_
#define KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUETYPE_H_
...
/**
 * Declaration MeasurementValueType
 */
class MeasurementValueType : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 5;

  //! Enumeration MeasurementValueType::Value
  enum class Value {
    UNDEFINED__ = -1,
    VALUE,
    AVERAGE_VALUE,
   ...
   */
  MeasurementValueType(MeasurementValueType::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  MeasurementValueType(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static MeasurementValueType fromString(const std::string &c_text);
   ...
   */
  MeasurementValueType::Value getValue() const;
   ...
   * @return enumeration value
   */
  MeasurementValueType::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const MeasurementValueType &other) const;
...
private:
  ...
  MeasurementValueType::Value m_data;
...
/**
 * \deprecated Use MeasurementValueType instead.
 */
using MeasurementValueTypeStruct = MeasurementValueType;
...
#endif // KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUETYPE_H_