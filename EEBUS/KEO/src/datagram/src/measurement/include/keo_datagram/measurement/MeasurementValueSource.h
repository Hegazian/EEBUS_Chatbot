...
#ifndef KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESOURCE_H_
#define KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESOURCE_H_
...
/**
 * Declaration MeasurementValueSource
 */
class MeasurementValueSource : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 3;

  //! Enumeration MeasurementValueSource::Value
  enum class Value {
    UNDEFINED__ = -1,
    MEASURED_VALUE,
    CALCULATED_VALUE,
   ...
   */
  MeasurementValueSource(MeasurementValueSource::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  MeasurementValueSource(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static MeasurementValueSource fromString(const std::string &c_text);
   ...
   */
  MeasurementValueSource::Value getValue() const;
   ...
   * @return enumeration value
   */
  MeasurementValueSource::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const MeasurementValueSource &other) const;
...
private:
  ...
  MeasurementValueSource::Value m_data;
...
/**
 * \deprecated Use MeasurementValueSource instead.
 */
using MeasurementValueSourceStruct = MeasurementValueSource;
...
#endif // KEO_DATAGRAM_MEASUREMENT_MEASUREMENTVALUESOURCE_H_