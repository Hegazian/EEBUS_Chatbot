 ...
 ******************************************************************************
 */
class AlarmDatagramPlugin : public KeoDatagramPlugin {
public:
  AlarmDatagramPlugin();
  virtual ~AlarmDatagramPlugin();
...
KEO_DATAGRAM_PLUGIN(AlarmDatagramPlugin, "Alarm Datagram Plugin",
                    SPINE_VERSION_STRING, SPINE_VERSION_POSTFIX)

AlarmDatagramPlugin::AlarmDatagramPlugin() {
...
extern "C" KeoDatagramPlugin *createPlugin() { return new AlarmDatagramPlugin; }

extern "C" void destroyPlugin(KeoDatagramPlugin *p) { delete p; }

CmdDataPtr AlarmDatagramPlugin::createCmdData(const CmdData::Type type) const {
  switch (type) {
...
DataSelectorsPtr
AlarmDatagramPlugin::createDataSelectors(const DataSelectors::Type type) const {
  switch (type) {
  case DataSelectors::Type::ALARM_LIST_DATA_SELECTORS_TYPE:
    return std::make_shared<AlarmListDataSelectors>();
...
DataElementsPtr
AlarmDatagramPlugin::createDataElements(const DataElements::Type type) const {
  switch (type) {
  case DataElements::Type::ALARM_DATA_ELEMENTS_TYPE:
    return std::make_shared<AlarmDataElements>();
