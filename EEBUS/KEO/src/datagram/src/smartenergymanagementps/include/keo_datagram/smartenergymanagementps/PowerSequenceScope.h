...
#ifndef KEO_DATAGRAM_SMARTENERGYMANAGEMENTPS_POWERSEQUENCESCOPE_H_
#define KEO_DATAGRAM_SMARTENERGYMANAGEMENTPS_POWERSEQUENCESCOPE_H_
...
/**
 * Declaration PowerSequenceScope
 */
class PowerSequenceScope : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 3;

  //! Enumeration PowerSequenceScope::Value
  enum class Value { UNDEFINED__ = -1, FORECAST, MEASUREMENT, RECOMMENDATION };
   ...
   */
  PowerSequenceScope(PowerSequenceScope::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  PowerSequenceScope(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static PowerSequenceScope fromString(const std::string &c_text);
   ...
   */
  PowerSequenceScope::Value getValue() const;
   ...
   * @return enumeration value
   */
  PowerSequenceScope::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const PowerSequenceScope &other) const;
...
private:
  ...
  PowerSequenceScope::Value m_data;
...
/**
 * \deprecated Use PowerSequenceScope instead.
 */
using PowerSequenceScopeStruct = PowerSequenceScope;
...
#endif // KEO_DATAGRAM_SMARTENERGYMANAGEMENTPS_POWERSEQUENCESCOPE_H_