...
#ifndef KEO_DATAGRAM_LOADCONTROL_LOADCONTROLCATEGORY_H_
#define KEO_DATAGRAM_LOADCONTROL_LOADCONTROLCATEGORY_H_
...
/**
 * Declaration LoadControlCategory
 */
class LoadControlCategory : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 3;

  //! Enumeration LoadControlCategory::Value
  enum class Value {
    UNDEFINED__ = -1,
    OBLIGATION,
    RECOMMENDATION,
   ...
   */
  LoadControlCategory(LoadControlCategory::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  LoadControlCategory(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static LoadControlCategory fromString(const std::string &c_text);
   ...
   */
  LoadControlCategory::Value getValue() const;
   ...
   * @return enumeration value
   */
  LoadControlCategory::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const LoadControlCategory &other) const;
...
private:
  ...
  LoadControlCategory::Value m_data;
...
/**
 * \deprecated Use LoadControlCategory instead.
 */
using LoadControlCategoryStruct = LoadControlCategory;
...
#endif // KEO_DATAGRAM_LOADCONTROL_LOADCONTROLCATEGORY_H_