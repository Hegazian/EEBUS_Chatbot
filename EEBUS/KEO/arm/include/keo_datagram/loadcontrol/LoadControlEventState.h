...
#ifndef KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTSTATE_H_
#define KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTSTATE_H_
...
/**
 * Declaration LoadControlEventState
 */
class LoadControlEventState : public ExtendableSpineEnum {
public:
  //! Number of possible enumeration values
  const static std::size_t MAX_ENUM = 6;

  //! Enumeration LoadControlEventState::Value
  enum class Value {
    UNDEFINED__ = -1,
    EVENT_ACCEPTED,
    EVENT_STARTED,
   ...
   */
  LoadControlEventState(LoadControlEventState::Value value);

  /**
   * Constructs an instance and sets its value from string.
   * @param value the value of the new instance
   */
  LoadControlEventState(const std::string &value);

  /**
   * Constructs the enumeration value from string.
   * @returns An enumeration object created from the string
   */
  static LoadControlEventState fromString(const std::string &c_text);
   ...
   */
  LoadControlEventState::Value getValue() const;
   ...
   * @return enumeration value
   */
  LoadControlEventState::Value getData() const;

  /**
   * Comparison operator ==
   * @param other object to compare
   */
  bool operator==(const LoadControlEventState &other) const;
...
private:
  ...
  LoadControlEventState::Value m_data;
...
/**
 * \deprecated Use LoadControlEventState instead.
 */
using LoadControlEventStateStruct = LoadControlEventState;
...
#endif // KEO_DATAGRAM_LOADCONTROL_LOADCONTROLEVENTSTATE_H_